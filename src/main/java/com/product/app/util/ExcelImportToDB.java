	...
	public UploadFileResponseDto loadDataToDB(MultipartFile file) {
		...
		try {
			...
			while (rowIterator.hasNext()) {
				...
				ProductCategory productCategory0 = productCategoryRepository
						.findDistinctByCategoryName(productCategoryRepo.getCategoryName());
				Optional<ProductCategory> productCategory = Optional.ofNullable(productCategory0);

				ProductCategory productCategory2;
				if (!productCategory.isPresent()) {
					productCategory2 = productCategoryRepository.save(productCategoryRepo);
				} else {
					productCategory2 = productCategory.get();
